...
/**
 * Probe whether firmware accepts creating a mesh interface.
 *
 * This API performs a minimal add/remove cycle of a mesh interface type to validate support.
 * It does not configure or join a mesh network.
 *
 * @note Channel list must be configured before calling this function.
 *
 * @return @ref MMWLAN_SUCCESS if mesh interface add/remove succeeded, else an appropriate
 *         error code.
 */
enum mmwlan_status mmwlan_mesh_interface_probe(void);

/** Queue an immediate supplicant scan for mesh peer discovery and admission. */
enum mmwlan_status mmwlan_mesh_request_scan(void);
...
/**
 * Optional policy callback for mesh peer admission.
 *
 * This is invoked after a mesh beacon has been delivered to @ref mmwlan_scan_rx_cb_t
 * and before the supplicant starts peer authentication. Return @c true to allow
 * admission, or @c false to keep the transmitter as a discovery-only peer.
 */
typedef bool (*mmwlan_mesh_peer_admission_cb_t)(const struct mmwlan_scan_result *result,
                                                void *arg);
...
struct mmwlan_sta_args
{
    ...
    void *scan_rx_cb_arg;
    /** Optional policy callback used before admitting a discovered mesh peer. */
    mmwlan_mesh_peer_admission_cb_t mesh_peer_admission_cb;
    /** Opaque argument to be passed to @ref mesh_peer_admission_cb. */
    void *mesh_peer_admission_cb_arg;
    /**
    ...
    /**
     * Enable 802.11s mesh mode for this connection profile.
     *
     * When enabled, the STA profile will be translated to mesh mode in the supplicant
     * configuration (equivalent to OpenWrt @c mode=mesh semantics).
     */
    bool mesh_mode;
    /**
     * Advertise the local mesh beacon only. When enabled, the interface does
     * not admit peers or pass received management frames to the supplicant.
     */
    bool mesh_beacon_only;
    /** Fixed mesh bootstrap centre frequency in kHz. Zero keeps scan-selected behavior. */
    uint32_t mesh_frequency_khz;
    /** Fixed mesh bootstrap operating bandwidth in MHz. */
...
#define MMWLAN_STA_ARGS_INIT                                           \
    {                                                                  \
        ...
        .scan_rx_cb = NULL,                                            \
        .scan_rx_cb_arg = NULL,                                        \
        .mesh_peer_admission_cb = NULL,                                \
        .mesh_peer_admission_cb_arg = NULL,                            \
...
/** Retrieve the latest RSSI measured from a directly connected mesh peer. */
enum mmwlan_status mmwlan_get_mesh_peer_rssi(const uint8_t *peer_addr,
                                              int16_t *rssi_dbm);

/** Retrieve an authorized mesh peer and its latest RSSI by authorized-peer index. */
enum mmwlan_status mmwlan_get_mesh_authorized_peer(uint8_t peer_index,
                                                    uint8_t *peer_addr,
                                                    int16_t *rssi_dbm,
                                                    bool *rssi_valid);

/**
 * Resolve the authorized radio next hop for a mesh destination.
 *
 * Direct peers resolve to themselves. Multi-hop destinations resolve through
 * the HWMP forwarding table learned from PREQ/PREP action frames. Expired
 * routes and routes through peers whose controlled port is closed are not
 * returned.
 *
 * @param destination  Final mesh destination MAC address.
 * @param next_hop     Buffer to receive the directly connected next-hop MAC.
 *
 * @return @ref MMWLAN_SUCCESS if a usable next hop was found,
 *         @ref MMWLAN_NOT_FOUND if no route is currently known, or
 *         @ref MMWLAN_INVALID_ARGUMENT for an invalid address or buffer.
 */
enum mmwlan_status mmwlan_mesh_get_next_hop(const uint8_t *destination,
                                             uint8_t *next_hop);

/** Return the number of mesh peers whose controlled port is authorized. */
uint8_t mmwlan_get_mesh_authorized_peer_count(void);

/**
 * Return whether the supplicant/driver mesh interface has completed setup.
 * This does not require SAE peering or an authorized peer.
 */
bool mmwlan_mesh_is_initialized(void);
 ...
 * @param bssid     Transmitter/BSSID address from the received management frame.
 * @param arg       Reference to the opaque argument provided with the filter.
 */
typedef void (
    *mmwlan_beacon_vendor_ie_filter_cb_t)(const uint8_t *ies,
                                         uint32_t ies_len,
                                         const uint8_t *bssid,
                                         void *arg);
...
/**
 * Replace the Vendor Specific IEs appended to locally transmitted mesh beacons.
 *
 * This is only valid while an MMWLAN mesh connection is active. A copy of the
 * supplied IE block is made before this function returns.
 *
 * @param ies      Complete sequence of EID/length/payload records, or NULL to clear.
 * @param ies_len  Length of @p ies, or zero to clear.
 *
 * @return @ref MMWLAN_SUCCESS on success, else an appropriate error code.
 */
