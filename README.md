...
`live-stocking-nrf54l15.hex` for the nRF54L15/MM6108 FGH100M carrier, and
`live-stocking-fgh200m.hex` for the nRF54L15/MM8108 FGH200M carrier. Mobile
produces `live-stocking-signed.apk` and `live-stocking-unsigned.apk`. These six
files are attached to a GitHub Release; the workflow-run artifacts contain the
same files. Both nRF54L15 jobs use the source-free, dual-chip package checked
into `firmware/modules/mm-iot-zephyr-prebuilt`; it selects MM6108 for FGH100M
and MM8108 for FGH200M without another repository checkout.
